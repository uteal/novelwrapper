...
export default (novelTag, factories, {
  ...
  const renderText = (str, tags, pauses, stepTime, keepPrevious) => {
  ...
  const parseStringWithTags = (str, pauses = []) => {
    ...
    for (const obj of str.matchAll(/\[(.*?)::(.*?)\]/g)) {
      ...
      pauses.splice(index - shift + 1, tag.length + 3);
      chars.splice(index - shift, tag.length + 3);
      tags.splice(index - shift, tag.length + 3);
      pauses.splice(index - shift + word.length, 1);
      chars.splice(index - shift + word.length, 1);
      tags.splice(index - shift + word.length, 1);
