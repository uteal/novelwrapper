...
export default (novelTag, factories, {
  ...
  class Novel {
    ...
    constructor() {
      ...
      $stories.addEventListener('pointerup', onStoriesClick);
      $dialogs.addEventListener('pointerup', onDialogsClick);
      $select.addEventListener('pointerup', onSelectClick);
    ...
    return {
      onWatcherCreate(eventName) {
        document.querySelectorAll(`[${watchedAttr}="${eventName}"]`).forEach((el) => {
          ...
          el.addEventListener('pointerup', onWatchedElementClick);
          elems.push(el);
        });
      },
      onWatcherRemove(eventName) {
        for (let i = elems.length - 1; i >= 0; i--) {
          ...
          if (el.getAttribute(watchedAttr) === eventName) {
            ...
            el.removeEventListener('pointerup', onWatchedElementClick);
            elems.splice(i, 1);
          }
        }
      },
