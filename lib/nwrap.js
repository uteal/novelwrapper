/*! NovelWrapper v0.5.1 | (c) 2025 uteal | MIT License */
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        await renderText(...parseStringWithTags(str, pauses), false, (blocks) => {
          for (const $elem of blocks) {
            $elem.classList.add(__ + '-paragraph-' + modifier);
          }
          $message.replaceChildren(...blocks);
        });
        ...
        dialogsCallback = () => {
          $message.classList.remove(__ + '-message-waiting');
          resolve();
        };
        ...
        if (SKIP_TEXT_ANIMATION) {
          ...
          $message.classList.add(__ + '-message-waiting');
          if (SKIP_TEXT_ANIMATION) {
            onDialogsClick(null, true);
          } else {
