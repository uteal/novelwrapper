/*! NovelWrapper v0.5.4 | (c) 2026 uteal | MIT License */
...
export default (novelTag, factories, {
  ...
  let onGameReady;
  const descriptorPromise = new Promise((resolve) => { onGameReady = resolve; });
  ...
  // Used for emergency exits from a scene (still a questionable practice).
  const resetExecutionVars = () => {

    watchersCallback = undefined;
    storiesCallback = undefined;
    dialogsCallback = undefined;
    choicesCallback = undefined;
  ...
  const FRAGMENT_CLASSES = {
    MESSAGE: __ + '-messageFragment',
    PASSAGE: __ + '-passageFragment',
    OPTION: __ + '-optionFragment'
  };
  ...
  const CALLBACKS = {
    ...
    /**
     * Fires when the scene is entered.
     * @param {string} id The scene's id.
     * @param {boolean} autosave Was the state just saved?
     */
    onSceneEnter: null,

    /**
     * Fires when the scene is called.
     * @param {string} id The scene's id.
     */
    onSceneCall: null,
    ...
    /**
     * Fires when the script flow started.
     */
    onScriptFlowStart: null,

    /**
     * Fires when the script flow ended.
     */
    onScriptFlowEnd: null,
     ...
     * @param {any} value Selected option value.
     */
    onOptionClick: null,
  ...
  const IMPLEMENTED = {
    ...
    async writeState(key, clonedState) {
      // log('💾', key, clonedState);
      if (useLocationHash) {
        let store = {};
  ...
  const onStoriesClick = (event, force = false) => {
    if (!GAME_ENDED && !event?.button) {
      ...
      } else if (!SCRIPT_FLOW && currentScene && event?.target.hasAttribute(pageLinkAttr)) {
  ...
  const onDialogsClick = (event, force = false) => {
    if (!GAME_ENDED && !event?.button && (WAITING_FOR_ACTION || force) && dialogsCallback) {
      WAITING_FOR_ACTION = false;
  ...
  const onSelectClick = async (event, optNum = undefined) => {
    if (GAME_ENDED || event?.button || !WAITING_FOR_ACTION || !choicesCallback) {
      return;
    }
    let target;
    if (typeof optNum === 'number') {
      ...
      target = event.target.closest('.' + __ + '-select-option');
      if (!target) {
        return;
      }
    }
    if (target.classList.contains(__ + '-select-option-locked')) {
      return;
    }
    WAITING_FOR_ACTION = false;
    const value = JSON.parse(target.getAttribute('select-option-value'));
    lastChoice.clear();
    lastChoice.set(String(value), true);
    lastChoice.set('__value__', value);
    const resolve = choicesCallback;
    choicesCallback = null;
    log('👉', value, '("' + target.innerText.replaceAll('\n', ' ') + '")');
    await CALLBACKS.onOptionClick?.(value);
    $select.classList.add(__ + '-select-hidden');
  ...
  class Novel {
    ...
    constructor() {
      ...
      $stories.addEventListener('pointerdown', onStoriesClick);
      $dialogs.addEventListener('pointerdown', onDialogsClick);
      $select.addEventListener('pointerdown', onSelectClick);
    ...
    async getReady() {
      const restored = !startOver && await this.loadState(restoreFromSlot);
    ...
    async saveState(slotName = 'autosave') {
      ASSERT_RELEVANCE();
      if (noSaveLoad) {
        return;
      }
      const key = IMPLEMENTED.createKey?.(novelTag, slotName);
      return await IMPLEMENTED.writeState?.(key, structuredClone(state));
    }
    ...
    async loadState(slotName) {
      ...
      const key = IMPLEMENTED.createKey?.(novelTag, slotName);
      return await IMPLEMENTED.readState?.(key);
    }
    ...
    async runScene(id, args = [], autosave = true) {
      ...
      if (id?.[0] === '~') {
        ...
        Object.values(characters).forEach((ch) => {
          if (Object.hasOwn(state.characters, ch.id)) {
            ch.setAlign(state.characters[ch.id].align);
            ch.setLabel(state.characters[ch.id].label);
          }
        });
      }
      if (!Object.hasOwn(scenes, id)) {
      ...
      if (autosave) {
        await this.saveState('autosave');
      }
      lastChoice.clear();
      await CALLBACKS.onSceneEnter?.(id, autosave);
      let result;
      try {
        result = structuredClone(await currentScene.run(...args));
      } catch (e) {
        if (Array.isArray(e)) {
          result = structuredClone(e);
        } else if (e !== ABORT_SIGNAL) {
          throw e;
        }
      }
    ...
    async notify(event, ...args) {
      ...
      if (watcher) {
        ...
        try {
          ...
          if (e === ABORT_SIGNAL) {
            log('✋', 'The watcher execution was forcibly interrupted.');
          } else {
            log('❌', 'Unexpected error during watcher execution.');
            throw e;
          }
  ...
  class Scene {
    ...
    async run(...args) {
      ...
      let thrown;
      try {
        result = await this.cb(charactersProxy, ...args);
      } catch (e) {
        thrown = e;
        // The (almost) only way to *run* a scene from the *called* scene:
        // throw [sceneName, ...sceneArgs];
        if (Array.isArray(e)) {
          log('🐾', 'A full-fledged scene has been invoked from the called scene.');
        } else if (e === ABORT_SIGNAL) {
          log('✋', 'The scene was forcibly interrupted.');
        } else {
          log('❌', 'Unexpected error during scene execution.');
          throw e;
        }
        log('🪂', 'Emergency exiting.');
        resetExecutionVars();
  ...
  class Character {
    ...
    async say(modifier, ...args) {
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        await renderText(...parseStringWithTags(str, pauses), FRAGMENT_CLASSES.MESSAGE, (blocks) => {
  ...
  // #region renderText
  const renderText = (str, tags, pauses, fragmentClass, onParagraphs) => {
    ...
    const makeLinks = fragmentClass === FRAGMENT_CLASSES.PASSAGE;
  ...
  const select = async (...args) => {
    ...
    args = args
      .map((arg, i) => (
        (arg === undefined || arg === null || arg === false)
          ? undefined
          : arg instanceof HTMLElement
            ? { elem: arg, value: i }
            : arg instanceof Object
              ? { ...arg, label: arg.label ?? '', value: arg.value ?? i }
              : { label: String(arg), value: i }
      ))
    ...
    if (currentScene?.id) {
      $select.setAttribute('scene', currentScene.id);
    } else {
      $select.removeAttribute('scene');
    }
    for (let opt of args) {
      const $option = div(__ + '-select-option');
      $option.setAttribute('select-option-value', JSON.stringify(opt.value));
      if (opt.lock) {
        $option.classList.add(__ + '-select-option-locked');
      }
      if (opt.elem) {
        $option.append(opt.elem);
        $option.classList.add(__ + '-select-option-custom');
      } else {
        renderText(...parseStringWithTags(...stripPauses(translate(String(opt.label)))), FRAGMENT_CLASSES.OPTION, (blocks) => {
  ...
  const __write = async (modifier, ...args) => {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      await renderText(...parseStringWithTags(...stripPauses(translate(messages[i]))), FRAGMENT_CLASSES.PASSAGE, (blocks) => {
        for (const $elem of blocks) {
  ...
  function onWatchedElementClick({ button }) {
    if (button === 0) {
    ...
    return {
      onWatcherCreate(eventName) {
        document.querySelectorAll(`[${watchedAttr}="${eventName}"]`).forEach((el) => {
          ...
          el.addEventListener('pointerdown', onWatchedElementClick);
          elems.push(el);
        });
      },
      onWatcherRemove(eventName) {
        for (let i = elems.length - 1; i >= 0; i--) {
          ...
          if (el.getAttribute(watchedAttr) === eventName) {
            ...
            el.removeEventListener('pointerdown', onWatchedElementClick);
            elems.splice(i, 1);
          }
        }
      },
  ...
  const call = (sceneId, ...args) => {
    ...
    return (async () => {
      await CALLBACKS.onSceneCall?.(sceneId);
      const result = structuredClone(await scenes[sceneId].cb(charactersProxy, ...args));
      callStackSize -= 1;
      if (!callStackSize) {
        log('💬→⛰️', currentScene?.id, '↩');
      }
  ...
  Object.assign(descriptor, {
    ...
    event: (eventName, ...args) => {
      novel.notify(eventName, ...args);
    },

    goTo: async (sceneId, ...args) => {
      if (!SCRIPT_FLOW) {
        await novel.runScene(sceneId, args);
      } else {
        console.error(`Cannot go to scene "${sceneId}" until the script flow is ended.`);
      }
    },

    forceTo: async (sceneId, ...args) => {
      if (SCRIPT_FLOW) {
    ...
    getCurrentSceneId: () => currentScene?.id,
