/*! NovelWrapper v0.4.1 | (c) 2025 uteal | MIT License */
...
export default (novelTag, factories, {
  ...
  const CALLBACKS = {
     ...
     * @returns {number|undefined} After-print delay in milliseconds.
     */
    onBeforeType: null,
      ...
      const printChar = (i) => {
        if (SKIP_PRINTING || stepTime === 0) {
          ...
          for (let j = 0; j < charNodes.length; j++) {
            CALLBACKS.onBeforeType?.(str[i + j], charNodes[j], tags[i + j], 0);
          }
          ...
          if (i < str.length) {
            ...
            setTimeout(() => {
              ...
              const t = CALLBACKS.onBeforeType?.(str[i], charNode, tags[i], currentStepTime);
              if (!t) {
