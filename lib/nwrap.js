/*! NovelWrapper v0.2.1 | (c) 2024 uteal | MIT License */

// Hint: If you don't want to use this file as a module, comment out
// the line starting with "export" and uncomment the line before it.

// window.createNovel = (novelTag, factories, initParams = {}) => {
export default (novelTag, factories, initParams = {}) => {
  ...
  const stdCallbacks = {
    ...
    // Fires when a character appears. Gets its id.
    onShow: null,

    // Fires when a character disappears. Gets its id.
    onHide: null,

    // Fires when a symbol is printed. Gets it.
    onType: null,

    // Fires when the text field is waiting for player's action.
    onStop: null,

    // Fires when the player entering a scene. Gets the scene id.
    onSceneEnter: null,

    // Fires when the player leaving a scene. Gets the scene id.
    onSceneLeave: null,

    // Fires when the watcher is created. Gets the watched event name.
    onWatcherCreate: null,

    // Fires when the watcher is removed. Gets the no longer watched event name.
    onWatcherRemove: null,

    // Fires when the player entering a watcher's callback. Gets the event name.
    onEventEnter: null,

    // Fires when the player leaving a watcher's callback. Gets the event name.
    onEventLeave: null,
    ...
    // Fires when a field is set in the storage object ($). Gets the property name and value.
    // Note: Changes to nested structures are not tracked.
    onStoreChange: null,

    // Fires when the game is ended. Gets the value that ended the game.
    onGameEnd: null
  ...
  const onSelectClick = async ({ target }) => {
    if (WAITING_FOR_ACTION && choicesCallback && target?.classList.contains(__ + '-select-option')) {
      ...
      await sleep(PAUSES.AFTER_OPTION_CLICK);
      CHOOSING = false;
      resolve(num);
    }
  };
  ...
  class Novel {
    ...
    constructor(id, {
      ...
      Object.assign(stdCallbacks, callbacks);
      DEV_MODE = devMode;
      STEP_TIME = stepTime;
    ...
    saveState(slotName) {
      ...
      const val = {
        ...
        characters: Object.fromEntries(Object.values(characters).map(
          ch => [ch.id, { align: ch.align, label: ch.label }]
        )),
        store
      };
    ...
    async runScene(id, autosave = true) {
      ...
      stdCallbacks.onSceneEnter?.(id);
      const result = await currentScene.run();
      stdCallbacks.onSceneLeave?.(id);
    ...
    async notify(event, ...args) {
      ...
      if (watcher) {
        ...
        stdCallbacks.onEventEnter?.(event);
        const result = await watcher.cb(...args);
        stdCallbacks.onEventLeave?.(event);
        if (typeof result === 'string') {
  ...
  class Character {
    ...
    constructor(id) {
      ...
      if (restoredData?.characters && Object.hasOwn(restoredData.characters, id)) {
        this.setLabel(restoredData.characters[id].label);
        this.move(restoredData.characters[id].align);
      }
    }
    ...
    async show(mood = this.mood) {
      ...
      if (!$portrait) {
        await new Promise((resolve) => {
          ...
          const source = `${imagesBasePath}${this.id}/${mood}.${imagesExtension}?${salt}`;
      ...
      if (!this.visible) {
        ...
        await sleep(PAUSES.CHARACTER_SHOW);
      } else {
        await doubleRAF(async () => {
          await sleep(moodChanged ? PAUSES.CHARACTER_MOOD_CHANGE : 0);
    ...
    async hide() {
      if (this.visible) {
        ...
        await sleep(PAUSES.CHARACTER_HIDE);
      }
    }
    ...
    async say(mood, ...args) {
      ...
      if (this) {
        ...
        if (delayNeeded) {
          await sleep(PAUSES.BEFORE_FIRST_PRINT);
        }
      }
      for (let i = 0; i < messages.length; i++) {
        ...
        await new Promise(async (resolve) => {
          ...
          if (FAST_FORWARD) {
            await sleep(PAUSES.BEFORE_FAST_FORWARD);
          }
          if (FAST_FORWARD) {
            onDialogsClick(true);
          } else if (auto) {
            ...
            await sleep(PAUSES.BEFORE_IDLE_PROCEED);
            onDialogsClick(true);
          } else {
  ...
  async function select(...args) {
    ...
    await sleep(PAUSES.BEFORE_SELECT_ACTIVE);
  ...
  // #region init
  novel = new Novel(novelTag, initParams);
