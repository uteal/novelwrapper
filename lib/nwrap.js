/*! NovelWrapper v0.3.3 | (c) 2024 uteal | MIT License */
...
export default (novelTag, factories, {
  ...
  const stateProxy = new Proxy(state, {
    set(target, prop, value) {
      if (prop.startsWith('__')) {
        console.error('Could not set property that starts with "__": such names are reserved by the engine.');
        return false;
      }
      target[prop] = value;
  ...
  class Novel {
    ...
    saveState(slotName) {
      ASSERT_RELEVANCE();
      IMPLEMENTED.saveState?.(slotName, structuredClone(state));
    }

    loadState(slotName) {
      ASSERT_RELEVANCE();
      return IMPLEMENTED.loadState?.(slotName);
    }

    clearSaveSlot(slotName) {
      ASSERT_RELEVANCE();
      IMPLEMENTED.clearSaveSlot?.(slotName);
    }
  ...
  class Character {
    ...
    constructor(id) {
      ...
      Object.assign($$, {
        $toLeft: () => {
          this.setAlign('left');
          state.__characters[id].align = 'left';
          return this.proxy_;
        },
        $toRight: () => {
          this.setAlign('right');
          state.__characters[id].align = 'right';
          return this.proxy_;
        },
        $setName: (str) => {
          this.setLabel(str);
          state.__characters[id].label = str;
          return this.proxy_;
        },
        $flipImage: () => {
          this.setFlip(!this.flipped);
          state.__characters[id].flipped = this.flipped;
          return this.proxy_;
        },
        then: () => {
          console.error(
      ...
      if (Object.hasOwn(state.__characters, id)) {
        const data = state.__characters[id];
        this.setLabel(data.label);
        this.setAlign(data.align);
        this.setFlip(data.flipped);
        log('↻', data);
      } else {
        state.__characters[id] = {};
      }
    }
  ...
  function log(...args) {
    if (DEV_MODE) {
      console.log(...args.map(arg => arg === stateProxy ? removeSpecialProps(state) : arg));
    }
  }

  function removeSpecialProps(obj) {
    return Object.fromEntries(Object.entries(obj).filter(a => !a[0].startsWith('__')));
  }
  // #endregion
  ...
  Object.assign(descriptor, {
    ...
    // and also unwraps and cleans up the game state object ($) for better view.
    log
