...
export default (novelTag, factories, {
  ...
  const CALLBACKS = {
     ...
     * @returns {(undefined|number)} Additional print time.
     */
    onBeforeType: null,
  ...
  const onSelectClick = async (arg) => {
    ...
    if (typeof arg === 'number') {
      targetIndex = arg;
      target = $select.querySelectorAll('.' + __ + '-select-option')[arg];
      if (!target) {
        return;
      }
    } else {
    ...
    for (const key of Object.keys(lastChoice)) {
      delete lastChoice[key];
    }
    lastChoice[value] = true;
    lastChoice.__value__ = value;
  ...
  class Character {
    ...
    async say(mood, ...args) {
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        await renderText(...parseStringWithTags(str, pauses), stepTime);
  ...
  const select = async (...args) => {
    ...
    $select.replaceChildren(...args.map((opt) => {
      ...
      for (let i = 0; i < charNodes.length; i++) {
        charNodes[i].style.pointerEvents = 'none';
  ...
  const selectProxy = new Proxy(select, {
    ...
    get(_target, prop) {
      if (Object.hasOwn(lastChoice, prop)) {
        return lastChoice[prop];
      }
    }
  });
  ...
  const renderText = (str, tags, pauses, stepTime) => {

    // console.log(str);
    // console.log(tags.map(v => v && '^' || '-').join(''));
    // console.log(pauses.map(v => v && '#' || '.').join(''));
    
    const charNodes = toCharNodes(str);

    $msgRendered.replaceChildren(); 
    $msgPlaceholder.append(...charNodes);

    return new Promise((resolve) => {
      const printChar = (i) => {
        if (SKIP_PRINTING || stepTime === 0) {
          for (let j = 0; j < charNodes.length; j++) {
            CALLBACKS.onBeforeType?.(str[j], charNodes[j], tags[j], 0, false);
          }
          $msgRendered.append(...charNodes);
          resolve();
        } else {
          if (i < pauses.length) {
            const currentStepTime = stepTime / (FAST_PRINTING ? 2 : 1);
            setTimeout(() => {
              let t;
              if (i < charNodes.length) {
                const charNode = charNodes[i];
                t = CALLBACKS.onBeforeType?.(str[i], charNode, tags[i], currentStepTime, false);
                $msgRendered.append(charNode);
              }
              if (t && typeof t === 'number') {
                setTimeout(() => printChar(i + 1), t);
              } else {
                printChar(i + 1);
              }
  ...
  const toCharNode = (char) => {
    const span = document.createElement('span');
    span.innerText = char;
    return span;
  };

  const toCharNodes = (str) => {
    const nodes = [];
  ...
  const parseStringWithTags = (str, pauses = []) => {
    ...
    let shift = 0;
    for (const obj of str.matchAll(/\[(.*?)::(.*?)\]/g)) {
