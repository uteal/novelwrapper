...
export default (novelTag, factories, {
  ...
  const CALLBACKS = {
     ...
     * @param {boolean} instantly Is text appearing instantly.
     */
    onType: null,
  ...
  class Character {
    ...
    async say(mood, ...args) {
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        await renderText(str, stepTime, directives.includes('+'));
  ...
  function renderText(text, stepTime, keepPrevious) {
    const [str, tags] = parseStringWithTags(translate(text));
    if (!keepPrevious) {
      $msgRendered.replaceChildren();
      ...
      const printChar = (i) => {
        if (SKIP_PRINTING || stepTime === 0) {
          ...
          for (let j = 0; j < charNodes.length; j++) {
            CALLBACKS.onType?.(str[i + j], charNodes[j], tags[i + j], true);
          }
          resolve();
        } else {
          if (i < str.length) {
            setTimeout(() => {
              const charNode = toCharNodes(str[i])[0];
              $msgRendered.append(charNode);
              if (!keepPrevious) {
                $msgPlaceholder.innerText = str.slice(i + 1);
              }
              CALLBACKS.onType?.(str[i], charNode, tags[i], false);
              printChar(i + 1);
